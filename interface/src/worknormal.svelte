<script>
    import { subjectHighlight , dateHighlight , worksSort } from "./store.js";
...
<div class="my-4 mx-10 bg-white py-8">

    {#each [...$worksSort] as [millionSecond , thisDate]}
        <div class="flex my-10 items-end">
            <span class="block w-2/6 pl-8">
                <button on:click={() => dateHighlight.set($dateHighlight===millionSecond ? "" : millionSecond)} class="bg-orange-100 rounded-md px-2 {$dateHighlight === millionSecond && "text-orange-600"}">
                    <span class="text-2xl mr-2">{numberToChinese[thisDate.dateDisplay.day]}</span>{thisDate.dateDisplay.month+1}/{thisDate.dateDisplay.date}
                </button>
            </span>


            <div class="w-4/6 flex items-center flex-wrap gap-y-2">

                {#each thisDate.child as work}
                    <span class="w-7/12">
                        {#if work.WorkOrExam}
                    ...
                    <span class="w-5/12">{work.range}</span>
                {/each}
